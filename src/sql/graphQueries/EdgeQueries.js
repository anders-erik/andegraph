...
async function selectEdgeFromId(edgeId) {
    return new Promise((acc, rej) => {
        graphDb.connection.all('SELECT * FROM edges WHERE id=?',
         [edgeId], 
         (err, rows) => {
...
async function insertEdge(edge) {
    

	let queryString = `
        INSERT INTO edges(id, node1, node2, directed, edgeOrder, edgePath)
		values(?, ?, ?, ?, ?, ?);
    `;
    ...
    return new Promise((acc, rej) => {
        graphDb.connection.run(
            queryString,
            [edge.id, edge.node1, edge.node2, edge.directed, edge.edgeOrder, edge.edgePath],
            (err, rows)  => {
                if (err) return rej(err);
                acc();
            },
...
async function checkIfEdgeIdExists(edgeId) {
    return new Promise((acc, rej) => {
        graphDb.connection.all('SELECT EXISTS(SELECT * FROM edges WHERE id=?)', 
        [edgeId], 
        (err, rows) => {
            if (err) return rej(err);
            console.log(rows)
            acc(Object.values(rows[0])[0]);
        });
    });
}
