...
async function selectReviewDates(sourceId) {
    return new Promise((acc, rej) => {
        graphDb.connection.all('SELECT * FROM sourceReviewDates WHERE sourceId=? ORDER BY date ASC', [sourceId], (err, rows) => {
...
async function selectReviewDateExists(date, sourceId) {
    return new Promise((acc, rej) => {
        graphDb.connection.all('SELECT EXISTS(SELECT 1 FROM sourceReviewDates WHERE date=? AND sourceId=?)', [date, sourceId], (err, rows) => {
            if (err) return rej(err);
            acc(
                rows.map(item =>
                    Object.assign({}, item, {
